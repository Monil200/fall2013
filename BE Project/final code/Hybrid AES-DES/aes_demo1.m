function [encrypt,decrypt]=aes_demo(msg,key,seed)
...
plaintext_len=length(plaintext);
   if(mod(length(plaintext),128)~=0)
        for i=1:128-mod(length(plaintext),128)
            plaintext(plaintext_len+i)=0;
        end
   end
    
arr_size=length(plaintext)/128;
start=1;
stop=128;
plaintextarray=[];
for i=1:arr_size
    plaintextarray(i,1:128)=plaintext(start:stop);%16*n array
     start=stop+1;
     stop=stop+128;
end
